import{t as e}from"./config-DDSCtEQs.js";var t=`${e}/api/admin`;function n(){return localStorage.getItem(`token`)}function r(){return{"Content-Type":`application/json`,Authorization:`Bearer ${n()}`}}function i(e){let t=document.getElementById(`admin-toast`);t.textContent=e,t.classList.add(`show`),clearTimeout(t._timer),t._timer=setTimeout(()=>t.classList.remove(`show`),3e3)}function a(e,t){document.getElementById(`modal-title`).textContent=e,document.getElementById(`modal-body`).innerHTML=t,document.getElementById(`modal`).classList.add(`open`),document.getElementById(`modal-overlay`).classList.add(`open`)}function o(){document.getElementById(`modal`).classList.remove(`open`),document.getElementById(`modal-overlay`).classList.remove(`open`)}function s(e,t){return`
  ...
  `}window.uploadFtImage=()=>{let t=document.createElement(`input`);t.type=`file`,t.accept=`image/*`,t.onchange=async()=>{let r=t.files[0];if(r){document.getElementById(`ft-upload-status`).textContent=`Uploading…`;try{let t=new FormData;t.append(`image`,r);let i=await(await fetch(`${e}/api/upload`,{method:`POST`,headers:{Authorization:`Bearer ${n()}`},body:t})).json();i.url?(document.getElementById(`ft-preview`).outerHTML=`<img src="${i.url}" id="ft-preview" style="width:100%;height:100%;object-fit:cover;" />`,document.getElementById(`ft-upload-status`).textContent=`Uploaded ✓`):document.getElementById(`ft-upload-status`).textContent=`Upload failed`}catch{document.getElementById(`ft-upload-status`).textContent=`Error`}}},t.click()},document.getElementById(`add-featured-btn`).addEventListener(`click`,()=>{F=null,a(`New Featured Type`,L(null)),document.getElementById(`ft-save-btn`).addEventListener(`click`,z),document.getElementById(`ft-featured`)?.addEventListener(`change`,R)}),window.editFeaturedType=e=>{F=e,a(`Edit Featured Type`,`<p class="muted" style="margin-bottom:0.75rem;">Loading…</p>`),u(`/featured-types`).then(t=>{let n=t.types.find(t=>t.id===e);if(!n){o(),i(`Not found`);return}document.getElementById(`modal-body`).innerHTML=L(n),document.getElementById(`ft-save-btn`).addEventListener(`click`,z),document.getElementById(`ft-featured`)?.addEventListener(`change`,R)}).catch(e=>{i(e.message),o()})};function R(){let e=document.getElementById(`ft-featured`),t=document.getElementById(`ft-featured-order-group`);t&&(t.style.display=e?.checked?`block`:`none`)}async function z(){let e=document.getElementById(`ft-preview`)?.getAttribute(`src`)||``,t=document.getElementById(`ft-name`).value.trim(),n=document.getElementById(`ft-desc`).value.trim(),r=parseInt(document.getElementById(`ft-order`).value)||0,a=document.getElementById(`ft-active`).checked,s=document.getElementById(`ft-featured`)?.checked||!1,c=parseInt(document.getElementById(`ft-featured-order`)?.value)||0;if(!t){i(`Name is required`);return}try{let l=JSON.stringify({name:t,description:n,imageUrl:e,order:r,active:a,featured:s,featuredOrder:c});F?(await u(`/featured-types/${F}`,{method:`PUT`,body:l}),i(`Updated`)):(await u(`/featured-types`,{method:`POST`,body:l}),i(`Added`)),o(),I()}catch(e){i(e.message)}}var B=null;async function V(){try{let e=await u(`/pincodes`),t=document.getElementById(`pincodes-container`);if(!e.pincodes||e.pincodes.length===0){t.innerHTML=`<div class="pincode-empty">No pincodes added yet. Click "+ Add Pincode" to begin.</div>`;return}t.innerHTML=e.pincodes.map(e=>`
    ...
    `).join(``)}catch(e){i(`Pincodes: `+e.message)}}window.deletePincode=async e=>{if(confirm(`Delete this pincode?`))try{await u(`/pincodes/${e}`,{method:`DELETE`}),i(`Pincode deleted`),V()}catch(e){i(e.message)}},document.getElementById(`pincode-add-btn`).addEventListener(`click`,()=>{B=null,a(`Add Pincode`,`
  ...
  `),document.getElementById(`pin-save`).addEventListener(`click`,H)}),window.editPincode=(e,t,n,r)=>{B=e,a(`Edit Pincode`,`
  ...
  `),document.getElementById(`pin-save`).addEventListener(`click`,H)};async function H(){let e=document.getElementById(`pin-code`).value.trim(),t=document.getElementById(`pin-area`).value.trim(),n=parseFloat(document.getElementById(`pin-charge`).value);if(!e||isNaN(n)){i(`Enter pincode and delivery charge`);return}try{B?(await u(`/pincodes/${B}`,{method:`PUT`,body:JSON.stringify({deliveryCharge:n,areaName:t})}),i(`Pincode updated`)):(await u(`/pincodes`,{method:`POST`,body:JSON.stringify({pincode:e,deliveryCharge:n,areaName:t})}),i(`Pincode added`)),o(),V()}catch(e){i(e.message)}}document.addEventListener(`DOMContentLoaded`,()=>{d(),f(),p(),y(),_(),v(),A(),V(),M(),I()});