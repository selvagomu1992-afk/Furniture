import{t as e}from"./config-DDSCtEQs.js";var t=`${e}/api/admin`;function n(){return localStorage.getItem(`token`)}function r(){return{"Content-Type":`application/json`,Authorization:`Bearer ${n()}`}}function i(e){let t=document.getElementById(`admin-toast`);t.textContent=e,t.classList.add(`show`),clearTimeout(t._timer),t._timer=setTimeout(()=>t.classList.remove(`show`),3e3)}function a(e,t){document.getElementById(`modal-title`).textContent=e,document.getElementById(`modal-body`).innerHTML=t,document.getElementById(`modal`).classList.add(`open`),document.getElementById(`modal-overlay`).classList.add(`open`)}function o(){document.getElementById(`modal`).classList.remove(`open`),document.getElementById(`modal-overlay`).classList.remove(`open`)}function s(e,t){return`
  ...
  `),c(`col-img`),document.getElementById(`collection-form`).addEventListener(`submit`,async e=>{e.preventDefault();try{await l(`/collections`,{method:`POST`,body:JSON.stringify({name:document.getElementById(`col-name`).value,slug:document.getElementById(`col-slug`).value,description:document.getElementById(`col-desc`).value,imageUrl:document.getElementById(`col-img-url`).value||null})}),o(),i(`Collection created`),d()}catch(e){i(e.message)}})}),window.editCollection=async e=>{try{let t=(await l(`/collections`)).collections.find(t=>t.id===e);if(!t)return;a(`Edit Collection`,`
      <form class="admin-form" id="collection-form">
    ...
    `),c(`col-img`),document.getElementById(`collection-form`).addEventListener(`submit`,async t=>{t.preventDefault();try{await l(`/collections/${e}`,{method:`PUT`,body:JSON.stringify({name:document.getElementById(`col-name`).value,slug:document.getElementById(`col-slug`).value,description:document.getElementById(`col-desc`).value,imageUrl:document.getElementById(`col-img-url`).value||null})}),o(),i(`Collection updated`),d()}catch(e){i(e.message)}})}catch(e){i(e.message)}},window.deleteCollection=async e=>{if(confirm(`Delete this collection? Products will be unlinked.`))try{await l(`/collections/${e}`,{method:`DELETE`}),i(`Collection deleted`),d()}catch(e){i(e.message)}};async function f(){try{let e=await l(`/products`),t=document.getElementById(`products-tbody`);t.innerHTML=e.products.map(e=>`
      <tr>
    ...
    `).join(``)}catch(e){i(`Products: ${e.message}`)}}var p=[];document.getElementById(`add-product-btn`).addEventListener(`click`,async()=>{try{p=(await l(`/collections`)).collections,a(`New Product`,m()),c(`prod-img`),document.getElementById(`product-form`).addEventListener(`submit`,h(`create`))}catch(e){i(e.message)}});function m(e){let t=[`LIVING`,`DINING`,`BEDROOM`,`OFFICE`],n=p.map(t=>`<option value="${t.id}" ${e?.collectionId===t.id?`selected`:``}>${t.name}</option>`).join(``);return`
  ...
  `}function h(e){return async t=>{t.preventDefault();let n={name:document.getElementById(`prod-name`).value,slug:document.getElementById(`prod-slug`).value,category:document.getElementById(`prod-category`).value,description:document.getElementById(`prod-desc`).value,basePrice:parseFloat(document.getElementById(`prod-price`).value)||0,collectionId:document.getElementById(`prod-collection`).value||null,imageUrl:document.getElementById(`prod-img-url`).value||null,badge:document.getElementById(`prod-badge`).value||null,inStock:document.getElementById(`prod-stock`).value===`true`};try{e===`create`&&(await l(`/products`,{method:`POST`,body:JSON.stringify(n)}),i(`Product created`)),o(),f()}catch(e){i(e.message)}}}window.editProduct=async e=>{try{let t=(await l(`/products/${e}`)).product;p=(await l(`/collections`)).collections,a(`Edit Product`,m(t)),c(`prod-img`),document.getElementById(`product-form`).addEventListener(`submit`,async t=>{t.preventDefault();let n={name:document.getElementById(`prod-name`).value,slug:document.getElementById(`prod-slug`).value,category:document.getElementById(`prod-category`).value,description:document.getElementById(`prod-desc`).value,basePrice:parseFloat(document.getElementById(`prod-price`).value)||0,collectionId:document.getElementById(`prod-collection`).value||null,imageUrl:document.getElementById(`prod-img-url`).value||null,badge:document.getElementById(`prod-badge`).value||null,inStock:document.getElementById(`prod-stock`).value===`true`};try{await l(`/products/${e}`,{method:`PUT`,body:JSON.stringify(n)}),o(),i(`Product updated`),f()}catch(e){i(e.message)}})}catch(e){i(e.message)}},window.deleteProduct=async e=>{if(confirm(`Delete this product?`))try{await l(`/products/${e}`,{method:`DELETE`}),i(`Product deleted`),f()}catch(e){i(e.message)}};async function g(){try{let e=await l(`/rates`);document.getElementById(`rate-delivery`).value=e.rates.delivery_rate||``,document.getElementById(`rate-tax`).value=e.rates.tax_rate||``,document.getElementById(`rate-minimum`).value=e.rates.minimum_order||``}catch(e){i(`Rates: ${e.message}`)}}document.getElementById(`rates-form`).addEventListener(`submit`,async e=>{e.preventDefault();try{await l(`/rates`,{method:`PUT`,body:JSON.stringify({delivery_rate:document.getElementById(`rate-delivery`).value,tax_rate:document.getElementById(`rate-tax`).value,minimum_order:document.getElementById(`rate-minimum`).value})}),i(`Rates saved`)}catch(e){i(e.message)}});async function _(){try{let e=await l(`/address`);document.getElementById(`addr-name`).value=e.address.name||``,document.getElementById(`addr-address`).value=e.address.address||``,document.getElementById(`addr-phone`).value=e.address.phone||``,document.getElementById(`addr-email`).value=e.address.email||``}catch(e){i(`Address: ${e.message}`)}}document.getElementById(`address-form`).addEventListener(`submit`,async e=>{e.preventDefault();try{await l(`/address`,{method:`PUT`,body:JSON.stringify({company_name:document.getElementById(`addr-name`).value,company_address:document.getElementById(`addr-address`).value,company_phone:document.getElementById(`addr-phone`).value,company_email:document.getElementById(`addr-email`).value})}),i(`Address saved`)}catch(e){i(e.message)}});async function v(){try{let e=document.getElementById(`order-status-filter`).value,t=document.getElementById(`order-payment-filter`).value,n=(document.getElementById(`order-search`)?.value||``).toLowerCase().trim(),r=new URLSearchParams;e&&r.set(`status`,e),t&&r.set(`payment`,t);let i=await l(`/orders?${r}`),a=document.getElementById(`orders-tbody`),o=document.getElementById(`order-badge`);if(o)try{let e=(await l(`/orders?status=PENDING&limit=1`)).total||0;o.style.display=e>0?`inline-flex`:`none`}catch{o.style.display=`none`}let s=i.orders||[];if(n&&(s=s.filter(e=>(e.user?.firstName+` `+e.user?.lastName).toLowerCase().includes(n)||(e.referenceCode||``).toLowerCase().includes(n)||(e.user?.email||``).toLowerCase().includes(n))),s.length===0){a.innerHTML=`<tr><td colspan="9" style="text-align:center;padding:2rem;color:var(--walnut-light);">No orders found</td></tr>`;return}a.innerHTML=s.map(e=>`
      <tr>
    ...
    `).join(``)}catch(e){i(`Orders: ${e.message}`)}}window.viewOrder=async e=>{try{let t=(await l(`/orders/${e}`)).order,n=t.items.map(t=>`
    ...
    `)}catch(e){i(e.message)}};var y=[`PENDING`,`CONFIRMED`,`IN_PRODUCTION`,`QUALITY_CHECK`,`SHIPPED`,`DELIVERED`,`CANCELLED`];window.updateStatus=async e=>{a(`Update Order Status`,`
  ...
  `),document.getElementById(`status-form`).addEventListener(`submit`,async t=>{t.preventDefault();try{await l(`/orders/${e}/status`,{method:`PATCH`,body:JSON.stringify({status:document.getElementById(`order-new-status`).value})}),o(),i(`Status updated`),v()}catch(e){i(e.message)}})},document.getElementById(`order-status-filter`).addEventListener(`change`,v),document.getElementById(`order-payment-filter`).addEventListener(`change`,v),document.getElementById(`order-search`)?.addEventListener(`input`,v),window.printInvoice=async e=>{try{let t=(await l(`/orders/${e}`)).order,n=await l(`/rates`),r=await l(`/address`),i=t.items.map((e,t)=>`
      <tr>
    ...
    `).join(``),a=t.items.reduce((e,t)=>e+t.quantity*t.price,0),o=parseFloat(n.rates?.delivery_rate||0),s=parseFloat(n.rates?.tax_rate||0),c=s/100*a,u=a+o+c,d=window.open(``,`_blank`);d.document.write(`
      <!DOCTYPE html>
      <html>
      ...
      <body>
        ...
        <table class="totals">
          ...
          <tr><td>Total</td><td>$${u.toFixed(2)}</td></tr>
        </table>
  ...
  `),document.getElementById(`company-invoice-form`).addEventListener(`submit`,async t=>{t.preventDefault(),o();let n=document.getElementById(`inv-company`).value,r=document.getElementById(`inv-gst`).value,i=document.getElementById(`inv-billing-addr`).value;await b(e,n,r,i)})};async function b(e,t,n,r){try{let i=(await l(`/orders/${e}`)).order,a=await l(`/rates`),o=await l(`/address`),s=i.items.map((e,t)=>`
    ...
    `).join(``),c=i.items.reduce((e,t)=>e+t.quantity*t.price,0),u=parseFloat(a.rates?.delivery_rate||0),d=parseFloat(a.rates?.tax_rate||0),f=d/100*c,p=c+u+f,m=window.open(``,`_blank`);m.document.write(`
      <!DOCTYPE html>
      <html>
      ...
      <body>
        ...
        <p class="amount-words">Amount in words: ${x(p)}</p>

        <table class="totals">
          <tr><td>Subtotal</td><td>$${c.toFixed(2)}</td></tr>
          ${u?`<tr><td>Delivery</td><td>$${u.toFixed(2)}</td></tr>`:``}
    ...
    `),m.document.close()}catch(e){i(`Company invoice: ${e.message}`)}}function x(e){if(e===0)return`Zero`;let t=[``,`One`,`Two`,`Three`,`Four`,`Five`,`Six`,`Seven`,`Eight`,`Nine`,`Ten`,`Eleven`,`Twelve`,`Thirteen`,`Fourteen`,`Fifteen`,`Sixteen`,`Seventeen`,`Eighteen`,`Nineteen`],n=[``,``,`Twenty`,`Thirty`,`Forty`,`Fifty`,`Sixty`,`Seventy`,`Eighty`,`Ninety`],r=[``,`Thousand`,`Million`,`Billion`];function i(e){if(e===0)return``;let r=``;return e>=100&&(r+=t[Math.floor(e/100)]+` Hundred `,e%=100),e>=20&&(r+=n[Math.floor(e/10)]+` `,e%=10),e>0&&(r+=t[e]+` `),r.trim()+` `}let a=``,o=0;for(;e>0;){let t=Math.floor(e%1e3);t>0&&(a=i(t)+r[o]+` `+a),e=Math.floor(e/1e3),o++}return a.trim()+` Dollars`}var S=[],C=[];async function w(){try{C=(await l(`/products`)).products||[];let e=document.getElementById(`cb-product-select`);e.innerHTML=`<option value="">— Select a product —</option>`+C.map(e=>`<option value="${e.id}" data-price="${e.basePrice}">${e.name} — $${e.basePrice}</option>`).join(``)}catch(e){i(`Counter Bill: ${e.message}`)}T()}document.getElementById(`cb-add-selected`).addEventListener(`click`,()=>{let e=document.getElementById(`cb-product-select`),t=e.value;if(!t){i(`Select a product`);return}let n=C.find(e=>e.id===t);if(!n)return;let r=S.find(e=>e.id===t);r?r.qty+=1:S.push({id:t,desc:n.name,qty:1,rate:n.basePrice}),e.value=``,T()});function T(){let e=document.getElementById(`cb-items-tbody`);if(S.length===0){e.innerHTML=`<tr><td colspan="5" style="text-align:center;padding:1.5rem;color:var(--walnut-light);font-size:0.85rem;">No items added yet. Select a product above.</td></tr>`,D();return}e.innerHTML=S.map((e,t)=>`
    <tr>
  ...
  `).join(``),document.querySelectorAll(`.cb-qty-inline`).forEach(e=>{e.addEventListener(`input`,()=>{let t=parseInt(e.dataset.index);S[t].qty=parseInt(e.value)||1,T()})}),document.querySelectorAll(`.cb-rate-inline`).forEach(e=>{e.addEventListener(`input`,()=>{let t=parseInt(e.dataset.index);S[t].rate=parseFloat(e.value)||0,T()})}),D()}window.removeCbItem=e=>{S.splice(e,1),T()};function E(){return{enabled:document.getElementById(`cb-gst-toggle`).checked,rate:parseFloat(document.getElementById(`cb-gst-rate`).value)||0}}async function D(){let e=S.reduce((e,t)=>e+t.qty*t.rate,0),t=E(),n=t.enabled?e*(t.rate/100):0,r=e+n;document.getElementById(`cb-subtotal`).textContent=`₹`+e.toFixed(2),document.getElementById(`cb-tax-row`).style.display=t.enabled?``:`none`,document.getElementById(`cb-tax`).textContent=`₹`+n.toFixed(2),document.getElementById(`cb-grand-total`).textContent=`₹`+r.toFixed(2)}document.getElementById(`cb-gst-toggle`).addEventListener(`change`,D),document.getElementById(`cb-gst-rate`).addEventListener(`input`,D),document.getElementById(`cb-print-btn`).addEventListener(`click`,async()=>{if(S.length===0){i(`Add at least one item`);return}await O(document.getElementById(`cb-customer-name`).value||`Walk-in Customer`,document.getElementById(`cb-customer-phone`).value||``)});async function O(e,t){try{let n=await l(`/address`),r=E(),i=S.reduce((e,t)=>e+t.qty*t.rate,0),a=r.enabled?i*(r.rate/100):0,o=i+a,s=a/2,c=a/2,u=`CB-`+Date.now().toString(36).toUpperCase(),d=S.map((e,t)=>`
      <tr>
    ...
    `).join(``),f=window.open(``,`_blank`);f.document.write(`
      <!DOCTYPE html>
      <html>
      <head><title>Counter Bill ${u}</title>
      <style>
        @page { margin: 12mm; }
        * { box-sizing:border-box; margin:0; padding:0; }
      ...
      <body>
        <div class="header">
          ...
          <div class="bill-no">Bill No: ${u} | ${new Date().toLocaleDateString(`en-US`,{year:`numeric`,month:`long`,day:`numeric`})}</div>
        </div>
    ...
    `),!n.read)try{await l(`/enquiries/${e}/read`,{method:`PATCH`}),k()}catch{}}catch(e){i(e.message)}},window.markRead=async e=>{try{await l(`/enquiries/${e}/read`,{method:`PATCH`}),i(`Marked as read`),o(),k()}catch(e){i(e.message)}},document.getElementById(`enquiry-filter`)?.addEventListener(`change`,k);var A=null;async function j(){try{let e=await l(`/pincodes`),t=document.getElementById(`pincodes-container`);if(!e.pincodes||e.pincodes.length===0){t.innerHTML=`<div class="pincode-empty">No pincodes added yet. Click "+ Add Pincode" to begin.</div>`;return}t.innerHTML=e.pincodes.map(e=>`
    ...
    `).join(``)}catch(e){i(`Pincodes: `+e.message)}}window.deletePincode=async e=>{if(confirm(`Delete this pincode?`))try{await l(`/pincodes/${e}`,{method:`DELETE`}),i(`Pincode deleted`),j()}catch(e){i(e.message)}},document.getElementById(`pincode-add-btn`).addEventListener(`click`,()=>{A=null,a(`Add Pincode`,`
  ...
  `),document.getElementById(`pin-save`).addEventListener(`click`,M)}),window.editPincode=(e,t,n,r)=>{A=e,a(`Edit Pincode`,`
  ...
  `),document.getElementById(`pin-save`).addEventListener(`click`,M)};async function M(){let e=document.getElementById(`pin-code`).value.trim(),t=document.getElementById(`pin-area`).value.trim(),n=parseFloat(document.getElementById(`pin-charge`).value);if(!e||isNaN(n)){i(`Enter pincode and delivery charge`);return}try{A?(await l(`/pincodes/${A}`,{method:`PUT`,body:JSON.stringify({deliveryCharge:n,areaName:t})}),i(`Pincode updated`)):(await l(`/pincodes`,{method:`POST`,body:JSON.stringify({pincode:e,deliveryCharge:n,areaName:t})}),i(`Pincode added`)),o(),j()}catch(e){i(e.message)}}document.addEventListener(`DOMContentLoaded`,()=>{u(),d(),f(),v(),g(),_(),k(),j()});